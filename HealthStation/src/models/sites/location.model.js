...
class quarantineLocation {
    ...
    async findAll() {
        const data = await db.any(`select * from $1 order by location_id`, this.table)
        return { data }
    }
    async findById(id) {
        const data = await db.one('select * from ${table} where location_id = ${id}', {
            table: this.table,
            id: id,
        });
        return { data };
    ...
    async deleteById(id) {
        ...
        const queryString =
            `delete from $(table) where location_id=$(id)`;
        await db.none(queryString, {
            table: this.table,
            id
        });
